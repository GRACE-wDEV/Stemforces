...
export const checkAndAwardAchievements = async (userId, userProgress, quizResult) => {
  ...
  try {
    ...
    const avgTimePerQuestion = (timeTaken * 60) / questionsTotal;
    ...
    const subjectProgressItem = userProgress.subject_progress.find(sp => sp.subject === subject);
    if (subjectProgressItem && subjectProgressItem.questions_correct >= 100) {
      ...
      if (!existingAchievement) {
        achievements.push({
          ...
          icon: '🧠',
          subject: subject,
          value: subjectProgressItem.questions_correct,
          is_rare: true,
          xp_reward: 150
        });
      }
